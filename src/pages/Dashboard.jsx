  ...
  return (
    <>
      ...
      <div className={style.container}>
        ...
        <p>We hope you're having a great day and we're so glad to have you back. Here's everything that's on your To Do List.<br />
        You can do this!</p>
        </div>
        ...
        <section className={style.todosection}>
          ...
          {openDailyTodos.length === 0 ? (
            <p>You're all done for today - thank you, for being a EcoQuest Hero!</p>
          ) : (
            <ul className={style.todoList}>
              {openDailyTodos.map((task, index) => (
                <li key={index} className={style.todoItem}>
                    ...
                    />
                    <span>
                        {task.text} (from:  <Link to={`/challengedetails/${task.challengeId}`}>{task.challengeTitle}</Link>)
                    </span>
                    </label>
                </li>
              ))}
